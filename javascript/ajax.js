...
function geocallback(datos){
	...
	var tmp = '<p><strong>'+barrio+'</strong><br>'+ciudad+',' +pais+'<br> ('+woeid+')</p>';
	$('#geo').prepend(tmp);
...
function climacallback(datos){
	...
	$('#clima').html('<strong>'+temp+'</strong> '+unit+'º').prepend(img);
}

function loadLogos(){
	$.get('javascript/ajax_html.html', function(html){
		$('footer').append(html);
	});
}
...
function loadUser(){
	$.get('javascript/ajax_json.json', function(datos){
